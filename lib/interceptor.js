...
var interceptor = {
  ...
  setup: function setup(done) {
    ...
    function addPendingRequest(startedRequest) {
      ...
      pushTolocalStorage(startedRequest);
    }
    ...
    function completeFetchRequest(startedRequest, completedRequest) {
      ...
      replaceInlocalStorage(startedRequest);
    }
    ...
    function getParsedlocalStorage() {
      // var rawData = window.localStorage.getItem(NAMESPACE);
      var rawData = window[NAMESPACE].requests;
      if (!rawData) {
        return [];
      }
      try {
        ...
        throw new Error(
          PKG_PREFIX + 'Could not parse localStorage data: ' + e.message
        );
      }
    }
    ...
    function pushTolocalStorage(req) {
      ...
      var parsed = getParsedlocalStorage();
      parsed.push(req);
      // window.localStorage.setItem(NAMESPACE, JSON.stringify(parsed));
    }

    function replaceInlocalStorage(completedRequest) {
      if (window[NAMESPACE].interceptorDisabled == true) {
        return;
      }
      if (!supportslocalStorage()) {
